...
const AUDIOBOOK_INDEX_CACHE_TTL_MS = Number(process.env.AUDIOBOOK_INDEX_CACHE_TTL_MS) || 5_000;
const audiobookIndexCache = new Map();
 ...
 * 获取有声书根目录
 */
function getAudiobookPath(context = null) {
  if (context?.audiobookPath) {
    if (!fs.existsSync(context.audiobookPath)) {
      fs.mkdirSync(context.audiobookPath, { recursive: true });
    }
    return context.audiobookPath;
  }
...
function getCacheKey(context = null) {
  const rootPath = getAudiobookPath(context);
  const userPart = context?.userId ? `user:${context.userId}` : 'legacy';
  return `${userPart}|path:${path.resolve(rootPath)}`;
}
...
function invalidateAudiobookIndexCache(context = null) {
  if (!context) {
    audiobookIndexCache.clear();
    return;
  }
  audiobookIndexCache.delete(getCacheKey(context));
}

/**
 * 加载元数据
 */
function loadMetadata(context = null) {
  if (context?.userId) {
    return getAppDb().listMetadata(context.userId);
  }
  try {
 ...
 * 保存元数据
 */
function saveMetadata(metadata, context = null) {
  if (context?.userId) {
    for (const [bookId, data] of Object.entries(metadata || {})) {
      getAppDb().updateMetadata(context.userId, bookId, data);
    }
    return;
  }
 ...
 * 更新书籍元数据
 */
function updateBookMetadata(contextOrBookId, bookIdOrUpdates, maybeUpdates) {
  const { context, bookId, updates } = resolveBookArgs(contextOrBookId, bookIdOrUpdates, maybeUpdates);
  if (context?.userId) {
    const updated = getAppDb().updateMetadata(context.userId, bookId, updates);
 ...
 * 获取书籍元数据
 */
function getBookMetadata(contextOrBookId, maybeBookId) {
  const { context, bookId } = resolveBookArgs(contextOrBookId, maybeBookId);
  if (context?.userId) {
    return getAppDb().getMetadata(context.userId, bookId);
  }
  const metadata = loadMetadata();
  return metadata[bookId] || {};
}

function resolveBookArgs(contextOrBookId, bookIdOrUpdates, maybeUpdates) {
  if ((typeof contextOrBookId === 'object' && contextOrBookId !== null) || (contextOrBookId === null && typeof bookIdOrUpdates === 'string')) {
    return { context: contextOrBookId, bookId: bookIdOrUpdates, updates: maybeUpdates };
  }
  return { context: null, bookId: contextOrBookId, updates: bookIdOrUpdates };
}
 ...
 * 扫描有声书目录
 */
function scanAudiobookFiles(context = null) {
  const rootPath = getAudiobookPath(context);
...
function scanAudiobooks(context = null) {
  const cacheKey = getCacheKey(context);
  const now = Date.now();
  const cached = audiobookIndexCache.get(cacheKey);
  if (cached && cached.expiresAt > now) {
    return cloneBooks(cached.books);
  }

  const books = scanAudiobookFiles(context);
  audiobookIndexCache.set(cacheKey, {
    books: cloneBooks(books),
    expiresAt: now + AUDIOBOOK_INDEX_CACHE_TTL_MS,
  });
 ...
 * 获取单本书详情
 */
function getBookDetail(contextOrBookId, maybeBookId) {
  const { context, bookId } = resolveBookArgs(contextOrBookId, maybeBookId);
  const books = scanAudiobooks(context);
 ...
 * 获取封面文件路径
 */
function getCoverPath(contextOrBookId, maybeBookId) {
  const { context, bookId } = resolveBookArgs(contextOrBookId, maybeBookId);
  const book = getBookDetail(context, bookId);
  if (!book) return null;
  
  const meta = getBookMetadata(context, bookId);
