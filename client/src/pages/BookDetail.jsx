...
import { getCachedAudioByBook, getPlayProgress } from '../utils/db';
...
export default function BookDetail() {
  ...
  const [cachedCountsBySeason, setCachedCountsBySeason] = useState({});
    ...
    (async () => {
      try {
        ...
        for (const season of book.seasons) {
          counts[season.id] = season.episodes.reduce((total, episode) => {
            const key = `${book.id}_${season.id}_${episode.id}`;
            return total + (cachedKeys.has(key) ? 1 : 0);
          }, 0);
        }

        if (!cancelled) {
          setCachedCountsBySeason(counts);
        }
      } catch (e) {
        console.error('Failed to load cached season counts:', e);
        if (!cancelled) {
          setCachedCountsBySeason({});
        }
      }
  ...
  const handleEpisodeCached = (seasonId) => {
    setCachedCountsBySeason(prev => {
      const targetSeason = book?.seasons.find(season => season.id === seasonId);
      const maxCount = targetSeason?.episodes.length || 0;
      const nextCount = Math.min((prev[seasonId] || 0) + 1, maxCount);
      return { ...prev, [seasonId]: nextCount };
    });
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-4">
        <button onClick={() => navigate(-1)} className="btn-ghost touch-target p-2">
          <HiArrowLeft className="w-6 h-6" />
        </button>
        <h1 className="text-lg font-semibold flex-1 truncate">{book.name}</h1>
        <button onClick={handleToggleFav} className="btn-ghost touch-target p-2">
          {isFav ? (
        ...
        <button onClick={() => setShowMetaEdit(!showMetaEdit)} className="btn-ghost touch-target p-2">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="field-ios resize-none"
              />
            </div>
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            {isDownloading && (
              <button
                ...
                className="touch-target text-xs text-red-400 hover:text-red-300 bg-red-500/10 px-2 py-1 rounded-lg transition-colors"
              >
                取消下载
              </button>
            )}
      ...
      <div className="space-y-3">
        {book.seasons.map((season, sIndex) => {
          const cachedCount = cachedCountsBySeason[season.id] || 0;
          const cacheLabel = cachedCount > 0
            ? `已缓存 ${cachedCount}/${season.episodes.length}`
            : '未缓存';
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-dark-400 mt-0.5">
                    ...
                    <span className="ml-2 text-green-500/80">{cacheLabel}</span>
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="touch-target text-dark-400 hover:text-primary-500 p-1 disabled:opacity-30"
                    title="缓存本季"
                    aria-label={`缓存本季：${season.name}`}
                  >
              ...
              {expandedSeason === sIndex && (
                ...
                >
                  <EpisodeList
                    ...
                    seasonIndex={sIndex}
                    onPlay={handlePlay}
                    onEpisodeCached={handleEpisodeCached}
                  />
                </motion.div>
              )}
            </div>
