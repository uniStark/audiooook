...
// ========== 每本书的转换进度 ==========
// userScopedBookId -> { total, completed, failed, failedFiles, currentFile, status: 'idle'|'converting'|'done'|'error' }
 ...
 * 如果已经在转换中则跳过
 */
function progressKey(bookId, scope = '') {
  return scope ? `${scope}:${bookId}` : bookId;
}

function startBookConversion(book, scope = '') {
  if (!book || !book.id) return;

  const key = progressKey(book.id, scope);
  const existing = bookProgress.get(key);
  ...
  bookProgress.set(key, progress);

  console.log(`[Converter] 开始转换: "${book.name}" (${files.length} 个文件)`);
  runConversionQueue(key, files, progress);
}
 ...
 * 获取某本书的转换进度
 */
function getConversionProgress(bookId, scope = '') {
  return bookProgress.get(progressKey(bookId, scope)) || null;
}
